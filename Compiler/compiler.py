...
def error(num):
	...
	if num == 1: 
		...
		print >>outfile, "Each case must use a constant or a number."
	elif num == 26: 
...
#add restricted globals to this, only change outside of rwords and statement
#-------------BLOCK------------------------------------------------
...
def statement(tx):
	...
	if sym == "ident":
		...
		getsym()
		if sym != "becomes":
			error(13) # := checked for
		getsym()
		expression(tx) #propperly assigned
...
rword.append('CEND')
norw=len(rword)
#--------------------done adding reserved words
#MAKE SURE TO UPDATE NUMBER OF RESERVED WORDS
